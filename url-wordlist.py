...
for line in user_file:
    ...
    for dir in url['directories']:
        clean = dir.strip('/')
        if clean not in dir_list:
            dir_list.append(clean)
    ...
    for x,y in query.items():
        combined = x + '=' + y[0]
        if combined not in query_list:
            query_list.append(combined)
            file_query.write(combined + '\n')
        if x not in query_key_list:
            query_key_list.append(x)
            file_query_key.write(x + '\n')
        if y[0] not in query_value_lists:
            query_value_lists.append(y[0])
